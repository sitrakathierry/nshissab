...
class MenuUserRepository extends ServiceEntityRepository
{
    ...
    public function allMenu($parent, $user)
    {
        ...
        $sql = "SELECT
                ...
                m.id,
                IF(m.route IS NULL,IF(u.roles LIKE '%ADMIN%','app_admin','app_home'),m.route) as route, 
    ...
    public function allMenuUserAgent($parent, $agence,$userId)
    {
        $conn = $this->getEntityManager()->getConnection();
        ...
        $sql = "SELECT
                ...
                FROM menu_agence ma
                JOIN menu m ON ma.menu_id = m.id 
                JOIN menu_user mu ON mu.menu_agence_id = ma.id
                WHERE $menuParent
                AND ma.agence_id = ? 
                AND mu.user_id = ?
                AND m.statut = 1 
        ...
        $stmt = $conn->prepare($sql);
        if(is_null($parent))
            $resultSet = $stmt->executeQuery([$agence,$userId]);
        else
            $resultSet = $stmt->executeQuery([$parent,$agence,$userId]);
